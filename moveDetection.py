...
class MoveDetector:
    ...
    def detectSquares(self):
        ...
        vertLines = np.vstack((
            cv2.HoughLines(edges, 1, np.pi / 180, 70, None, 0, 0, 0, 0.05*np.pi),
            cv2.HoughLines(edges, 1, np.pi / 180, 70, None, 0, 0, 0.95*np.pi, np.pi)
        ))
    ...
    def getMove(self):
        ...
        while True:
            ...
            if abs(crntNoise - self.avgNoise) < 0.5:
                stillCounter += 1
            else:
                stillCounter = 0
                hasMoved = True
            ...
            if stillCounter > 30 and hasMoved:
                ...
                crntPosition = {
                    key: self.getBox(frame, point)
                    for (key, point) in self.fieldPositions.items()
                }

                change = {
                    key: np.var(np.mean(
                        cv2.absdiff(crntPosition[key], lastPosition[key]),
                        (0,1)))
                ...
                candidates = sorted(
                    change.keys(),
                    key=lambda x: change[x],
                    reverse=True
                )

                if np.var(np.mean(crntPosition[candidates[0]], (0,1))) > \
                   np.var(np.mean(crntPosition[candidates[1]], (0,1))):
